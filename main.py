...
def parse_option():
    ...
    # parser.add_argument('--cfg', type=str, metavar="FILE", help='path to config file',
    #                    default='./configs/PN_KNN/CUB_res12_5way-5shot.yaml')
    parser.add_argument('--cfg', type=str, metavar="FILE", help='path to config file',
    ...
    # parser.add_argument('--pretrained', type=str, default='./pretrain/max_acc_dist.pth')
    ...
    parser.add_argument('--lam', default=0.7, help="KNN")
...
def train(config):
    ...
    if config.MODEL.PRETRAINED:
        ...
        #
        # #encoder = models.load(torch.load('./pretrain/mini-resnet12.pth')).encoder
        # pretrained_weights_path = './pretrain/mini-resnet12.pth'
        ...
        # tmp = torch.load('./pretrain/250.tar')
        #
        # state = tmp['state']
        ...
        #         # state.pop(key)
        #
        #         state.pop('classifier.L.weight_g', None)
        #         state.pop('classifier.L.weight_v', None)
        # state.pop('classifier.L.weight_g', None)
        # state.pop('classifier.L.weight_v', None)
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        sheet[f'A{epoch + 2}'] = epoch
        sheet[f'B{epoch + 2}'] = loss
        sheet[f'C{epoch + 2}'] = knn_acc
        sheet[f'D{epoch + 2}'] = proto_acc
        sheet[f'E{epoch + 2}'] = acc_current
...
def test(config):
    ...
    # with torch.no_grad():
    knn_acc, proto_acc, acc1, loss, ci,label_acc1,label_num1,label_acc2,label_num2,label_acc,label_num,label_num_total,than16_1,than16_2,than16,balance_acc_num1,balance_acc_num2,balance_acc_num3 = testing(config, test_dataset, test_dataloader,  model,optimizer,lr_scheduler,step)
    logger.info(
        f"Test Accuracy of {config.DATA.TEST.DATASET_NAMES[0]}: {knn_acc:.2f}%,{proto_acc:.2f}%,{acc1:.2f}%+-{ci:.2f}")
    logger.info(
...
def validate(config, dataset, data_loader, model, epoch=None, writer=None):
    ...
    for idx, batches in enumerate(data_loader):
        ...
        loss, knn_acc, proto_acc, acc = model.val_forward(imgs, labels, dataset_index)
    ...
    logger.info(f'knn_Acc {knn_acc_meter.avg:.2f}\t'
                f'proto_Acc {proto_acc_meter.avg:.2f}\t'
    ...
    return knn_acc_meter.avg, proto_acc_meter.avg, acc_meter.avg, loss_meter.avg
...
def testing(config, dataset, data_loader, model,optimizer=None,lr_scheduler=None,step=None):
    ...
    for idx, batches in enumerate(data_loader):
        ...
        loss, knn_acc, proto_acc, acc,label_acc1,label_num1,label_acc2,label_num2,label_acc,label_num,label_total_num,than16_1,than16_2,than16,balance_acc1,balance_acc2,balance_acc3= model.test_forward(imgs, labels, dataset_index,model = model,optimizer = optimizer,step = step)
    ...
    return knn_acc_meter.avg, proto_acc_meter.avg, acc_meter.avg, loss_meter.avg, ci,label_acc1_meter.avg,label_num1_meter,\
           ...
           than2_meter,than_meter,balance_acc_num1,balance_acc_num2,balance_acc_num3
